...
public class MapDBPersistenceManager extends
	BasePersistenceManager<MapDBClassInfo, MapDBQueryInfo<?>, TransScope, Map<String, Object>>
{
	...
	private final LoadingCache<MapDBClassInfo, Function<Map<String, Object>, ?>>					sTransformerToObjectMap		=
																																	CacheBuilder
																																		.newBuilder()
																																		.build(
																																			new CacheLoader<MapDBClassInfo, Function<Map<String, Object>, ?>>()
																																			{
	...
	private final LoadingCache<MapDBClassInfo, Function<Map<String, Object>, Map<String, Object>>>	sTransformerToKeysMap		=
																																	CacheBuilder
																																		.newBuilder()
																																		.build(
																																			new CacheLoader<MapDBClassInfo, Function<Map<String, Object>, Map<String, Object>>>()
																																			{
																																				public Function<Map<String, Object>, Map<String, Object>> load(
																																					MapDBClassInfo pClass)
																																					throws Exception
																																				{
	...
	protected TransScope openTransactionScope(boolean pImplicitTransaction)
	{
		...
		if (pImplicitTransaction == true)
		{
			if (scope.tData.db == null)
			{
				...
				{
					/*
					 * It's set to auto, so, if we've previously switched to transactional, then stay there, otherwise
					 * use a non-transaction db
					 */
					if (mTxMaker != null)
					{
						scope.tData.db = mTxMaker.makeTx();
		...
		else
		{
			if (scope.tData.db != null)
			{
				...
				else
				{
					...
					{
						/*
						 * We need to attempt to close the non-transaction database. We'll wait for up to N seconds, and
						 * prevent new ones from spawning
						 */
	 ...
	 */
	@Override
	protected void storeMapData(TransScope pScope, MapDBClassInfo pCLInfo, List<MapData<Map<String, Object>>> pData,
		boolean pIsInsert)
	{
	...
	/**
	 * @see siena.core.base.BasePersistenceManager#generateNextSequence(siena.mapdb.ExtClassInfo, java.lang.String)
	 */
	@Override
	protected long generateNextSequence(MapDBClassInfo pClassInfo, String pColumnName)
	{
		...
		{
			Atomic.Integer aInt =
				Atomic.getInteger(scope.tData.db, pClassInfo.mSienaInfo.tableName + "." + pColumnName + "__INC");
			int result = aInt.get();
			result = result + 1;
			aInt.set(result);
			scope.transResult = TransResultType.COMMIT;
	...
	/**
	 * @see siena.core.base.BasePersistenceManager#createQueryInfo(siena.core.base.ExtClassInfo, siena.BaseQueryData,
	 *      boolean)
	 */
	@Override
	...
	@SuppressWarnings("unchecked")
	@Override
	protected <T> T getLowColumnValue(Map<String, Object> pObj, MapDBClassInfo pCLInfo, Class<T> pResultClass,
		Field pField)
	{
	...
	@Override
	protected int compareRawToLowColumnValue(Map<String, Object> pObj, MapDBClassInfo pCLInfo, Field pField,
		Object pRawValue)
	{
		String colName = pCLInfo.mFieldToColumnNameMap.get(pField);
	...
	@Override
	protected void setLowColumnValue(Map<String, Object> pObj, MapDBClassInfo pCLInfo, Field pField,
