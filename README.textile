...
**Latest tagged version is Siena v1.0.0-b7**

h2. The diamondq fork

This fork provides a couple of additional features beyond the master:
* Broken up all the different data store implementations into separate projects to reduce dependency clutter
* Simplified the implementation of a new data store implementation by creating a strong abstract base class.
* Simplified the testing of a new implementation by enabling a common set of test cases
* Added the MapDB data store implementation

bq. You can now find some "manuals":https://github.com/diamondq/siena/wiki/Manuals

Siena is a persistence API for Java inspired by the Google App Engine Python Datastore trying to draw a bridge between SQL and NoSQL worlds. 
