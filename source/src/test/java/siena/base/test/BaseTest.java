...
public abstract class BaseTest extends TestCase
{
	...
	private static PersonLongManualID	LongManualID_TESLA		= new PersonLongManualID(1L, "Nikola", "Tesla",
																	"Smiljam", 1);
	private static PersonLongManualID	LongManualID_CURIE		= new PersonLongManualID(2L, "Marie", "Curie",
																	"Warsaw", 2);
	private static PersonLongManualID	LongManualID_EINSTEIN	= new PersonLongManualID(3L, "Albert", "Einstein",
																	"Ulm", 3);

	private static PersonStringID		StringID_TESLA			= new PersonStringID("TESLA", "Nikola", "Tesla",
																	"Smiljam", 1);
	private static PersonStringID		StringID_CURIE			= new PersonStringID("CURIE", "Marie", "Curie",
																	"Warsaw", 2);
	private static PersonStringID		StringID_EINSTEIN		= new PersonStringID("EINSTEIN", "Albert", "Einstein",
																	"Ulm", 3);
	...
	public void testFilterOperatorNotEqualString()
	{
		List<PersonStringID> people =
			pm.createQuery(PersonStringID.class).filter("firstName!=", "Albert").order("firstName").fetch();
	...
	public void testFilterOperatorNotEqualLongAutoID()
	{
		List<PersonLongAutoID> people =
			pm.createQuery(PersonLongAutoID.class).filter("id!=", LongAutoID_EINSTEIN.id).order("id").fetch();
	...
	public void testFilterOperatorNotEqualLongManualID()
	{
		List<PersonLongManualID> people =
			pm.createQuery(PersonLongManualID.class).filter("id!=", 3L).order("id").fetch();
	...
	{
		List<PersonStringID> people =
			pm.createQuery(PersonStringID.class).filter("id!=", StringID_EINSTEIN.id).order("id").fetch();
	...
	{
		@SuppressWarnings("serial")
		List<PersonUUID> people = pm.createQuery(PersonUUID.class).filter("n IN", new ArrayList<Integer>()
		{
			{
				add(2);
				add(3);
	...
	public void testFilterOperatorInForUUID()
	{
		...
		List<PersonUUID> people =
			pm.createQuery(PersonUUID.class).filter("id IN", Arrays.asList(l.get(0).id, l.get(1).id)).order("id")
				.fetch();
	...
	{
		@SuppressWarnings("serial")
		List<PersonLongAutoID> people = pm.createQuery(PersonLongAutoID.class).filter("id IN", new ArrayList<Long>()
		{
			{
	...
	public void testFilterOperatorInForLongManualID()
	{
		@SuppressWarnings("serial")
		List<PersonLongManualID> people =
			pm.createQuery(PersonLongManualID.class).filter("id IN", new ArrayList<Long>()
			{
				{
	...
	{
		@SuppressWarnings("serial")
		List<PersonStringID> people = pm.createQuery(PersonStringID.class).filter("id IN", new ArrayList<String>()
		{
			{
	...
	{
		List<PersonStringID> people =
			pm.createQuery(PersonStringID.class).filter("id<", StringID_TESLA.id).order("id").fetch();
	...
	{
		List<PersonLongManualID> people =
			pm.createQuery(PersonLongManualID.class).filter("id<=", LongManualID_EINSTEIN.id).order("id").fetch();
	...
	{
		List<PersonLongAutoID> people =
			pm.createQuery(PersonLongAutoID.class).filter("id>", LongAutoID_TESLA.id).order("id").fetch();
	...
	{
		List<PersonLongManualID> people =
			pm.createQuery(PersonLongManualID.class).filter("id>", LongManualID_TESLA.id).order("id").fetch();
	...
	{
		List<PersonStringID> people =
			pm.createQuery(PersonStringID.class).filter("id>", StringID_CURIE.id).order("id").fetch();
	...
	{
		List<PersonLongAutoID> people =
			pm.createQuery(PersonLongAutoID.class).filter("id>=", LongAutoID_CURIE.id).order("id").fetch();
	...
	{
		List<PersonLongManualID> people =
			pm.createQuery(PersonLongManualID.class).filter("id>=", LongManualID_CURIE.id).order("id").fetch();
	...
	public void testIterFullLongManualID()
	{
		...
		PersonLongManualID[] array =
			new PersonLongManualID[] {LongManualID_TESLA, LongManualID_CURIE, LongManualID_EINSTEIN};
	...
	public void testDataTypesNotNull()
	{
		...
		// Blob
		dataTypes.typeBlob =
			new byte[] {(byte) 0x01, (byte) 0x02, (byte) 0x03, (byte) 0x04, (byte) 0x10, (byte) 0X11, (byte) 0xF0,
					(byte) 0xF1, (byte) 0xF9, (byte) 0xFF};
	...
	public void testJoinAnnotation()
	{
		...
		List<Discovery4Join> res = pm.createQuery(Discovery4Join.class).order("id").fetch();
		assertEquals(4, res.size());
		assertEquals(radioactivity, res.get(0));
		assertEquals(relativity, res.get(1));
	...
	/*
	 * public void testFetchPaginate() { Discovery[] discs = new Discovery[10]; for(int i=0; i<10; i++){ discs[i] = new
	 * Discovery("Disc_"+i, LongAutoID_CURIE); pm.insert(discs[i]); } Query<Discovery> query =
	 * pm.createQuery(Discovery.class).paginate(5).order("name"); List<Discovery> res = query.fetch(); assertEquals(5,
	 * res.size()); for(int i=0; i<5; i++){ assertEquals(discs[i], res.get(i)); } res = query.fetch(); assertEquals(5,
	 * res.size()); for(int i=0; i<5; i++){ assertEquals(discs[i+5], res.get(i)); } } public void
	 * testFetchPaginateReuse() { Discovery[] discs = new Discovery[10]; for(int i=0; i<10; i++){ discs[i] = new
	 * Discovery("Disc_"+i, LongAutoID_CURIE); pm.insert(discs[i]); } Query<Discovery> query =
	 * pm.createQuery(Discovery.class).paginate(5).order("name").stateful(); try { List<Discovery> res = query.fetch();
	 * assertEquals(5, res.size()); for(int i=0; i<5; i++){ assertEquals(discs[i], res.get(i)); } res = query.fetch();
	 * assertEquals(5, res.size()); for(int i=0; i<5; i++){ assertEquals(discs[i+5], res.get(i)); } }catch(Exception
	 * ex){ fail(ex.getMessage()); } finally{ query.release(); } } public void testFetchKeysPaginate() { Discovery[]
	 * discs = new Discovery[10]; for(int i=0; i<10; i++){ discs[i] = new Discovery("Disc_"+i, LongAutoID_CURIE);
	 * pm.insert(discs[i]); } Query<Discovery> query = pm.createQuery(Discovery.class).paginate(5).order("name");
	 * List<Discovery> res = query.fetchKeys(); assertEquals(5, res.size()); for(int i=0; i<5; i++){
	 * assertEquals(discs[i].id, res.get(i).id); assertTrue(res.get(i).isOnlyIdFilled()); } res = query.fetchKeys();
	 * assertEquals(5, res.size()); for(int i=0; i<5; i++){ assertEquals(discs[i+5].id, res.get(i).id);
	 * assertTrue(res.get(i).isOnlyIdFilled()); } } public void testFetchKeysPaginateReuse() { Discovery[] discs = new
	 * Discovery[10]; for(int i=0; i<10; i++){ discs[i] = new Discovery("Disc_"+i, LongAutoID_CURIE);
	 * pm.insert(discs[i]); } Query<Discovery> query =
	 * pm.createQuery(Discovery.class).paginate(5).order("name").stateful(); try { List<Discovery> res =
	 * query.fetchKeys(); assertEquals(5, res.size()); for(int i=0; i<5; i++){ assertEquals(discs[i].id, res.get(i).id);
	 * assertTrue(res.get(i).isOnlyIdFilled()); } res = query.fetchKeys(); assertEquals(5, res.size()); for(int i=0;
	 * i<5; i++){ assertEquals(discs[i+5].id, res.get(i).id); assertTrue(res.get(i).isOnlyIdFilled()); } }finally {
	 * query.release(); } } public void testIterPaginate() { Discovery[] discs = new Discovery[10]; for(int i=0; i<10;
	 * i++){ discs[i] = new Discovery("Disc_"+i, LongAutoID_CURIE); pm.insert(discs[i]); } Query<Discovery> query =
	 * pm.createQuery(Discovery.class).paginate(5).order("name"); try { Iterable<Discovery> res = query.iter();
	 * Iterator<Discovery> it = res.iterator(); int i=0; while(it.hasNext()){ assertEquals(discs[i++], it.next()); } res
	 * = query.iter(); it = res.iterator(); while(it.hasNext()){ assertEquals(discs[i++], it.next()); } }finally {
	 * query.release(); } } public void testIterFetchPaginate() { Discovery[] discs = new Discovery[10]; for(int i=0;
	 * i<10; i++){ discs[i] = new Discovery("Disc_"+i, LongAutoID_CURIE); pm.insert(discs[i]); } Query<Discovery> query
	 * = pm.createQuery(Discovery.class).paginate(5).order("name"); try { Iterable<Discovery> res = query.iter();
	 * Iterator<Discovery> it = res.iterator(); int i=0; while(it.hasNext()){ assertEquals(discs[i++], it.next()); }
	 * List<Discovery> res2 = query.fetch(); assertEquals(5, res2.size()); for(int j=0; j<5; j++){
	 * assertEquals(discs[j+5], res2.get(j)); } }finally { query.release(); } } public void testSearchSingle() {
	 * Discovery4Search[] discs = new Discovery4Search[10]; for(int i=0; i<10; i++){ discs[i] = new
	 * Discovery4Search("Disc_"+i, LongAutoID_CURIE); pm.insert(discs[i]); } Query<Discovery4Search> query =
	 * pm.createQuery(Discovery4Search.class).search("Disc_5", "name"); List<Discovery4Search> res = query.fetch();
	 * assertEquals(1, res.size()); assertEquals(discs[5], res.get(0)); } public void testBatchInsert() { Object[] discs
	 * = new Discovery[10]; for(int i=0; i<10; i++){ discs[i] = new Discovery("Disc_"+i, LongAutoID_CURIE); }
	 * pm.insert(discs); List<Discovery> res = pm.createQuery(Discovery.class).fetch(); assertEquals(discs.length,
	 * res.size()); int i=0; for(Discovery disc:res){ assertEquals(discs[i++], disc); } } public void
	 * testBatchInsertList() { List<Discovery> discs = new ArrayList<Discovery>(); for(int i=0; i<10; i++){
	 * discs.add(new Discovery("Disc_"+i, LongAutoID_CURIE)); } pm.insert(discs); List<Discovery> res =
	 * pm.createQuery(Discovery.class).fetch(); assertEquals(discs.size(), res.size()); int i=0; for(Discovery
	 * disc:res){ assertEquals(discs.get(i++), disc); } } public void testBatchDelete() { Object[] discs = new
	 * Discovery[10]; for(int i=0; i<10; i++){ discs[i] = new Discovery("Disc_"+i, LongAutoID_CURIE);
	 * pm.insert(discs[i]); } List<Discovery> res = pm.createQuery(Discovery.class).fetch(); assertEquals(discs.length,
	 * res.size()); pm.delete(discs); res = pm.createQuery(Discovery.class).fetch(); assertEquals(0, res.size()); }
	 * public void testBatchDeleteList() { List<Discovery> discs = new ArrayList<Discovery>(); for(int i=0; i<10; i++){
	 * Discovery disc = new Discovery("Disc_"+i, LongAutoID_CURIE); discs.add(disc); pm.insert(disc); } List<Discovery>
	 * res = pm.createQuery(Discovery.class).fetch(); assertEquals(discs.size(), res.size()); pm.delete(discs); res =
	 * pm.createQuery(Discovery.class).fetch(); assertEquals(0, res.size()); } public void testBatchDeleteByKeys() {
	 * pm.deleteByKeys(PersonStringID.class, "TESLA", "CURIE"); List<PersonStringID> res =
	 * pm.createQuery(PersonStringID.class).fetch(); assertEquals(1, res.size()); assertEquals(StringID_EINSTEIN,
	 * res.get(0)); } public void testBatchDeleteByKeysList() { pm.deleteByKeys(PersonStringID.class, new
	 * ArrayList<String>(){{add("TESLA"); add( "CURIE");}}); List<PersonStringID> res =
	 * pm.createQuery(PersonStringID.class).fetch(); assertEquals(1, res.size()); assertEquals(StringID_EINSTEIN,
	 * res.get(0)); }
	 */
	...
	public void testBatchInsert()
	{
		...
		List<Discovery> res = pm.createQuery(Discovery.class).order("id").fetch();
	...
	public void testBatchDelete()
	{
		...
		res = pm.createQuery(Discovery.class).fetch();
	...
	public void testBatchDeleteByKeys()
	{
		...
		List<PersonStringID> res = pm.createQuery(PersonStringID.class).fetch();
	...
	public void testDumpQueryOption()
	{
		...
		assertNotNull(str);
		assertEquals(
			"{\"value\": {\"pageType\": \"TEMPORARY\", \"state\": \"PASSIVE\", \"pageSize\": 0, \"type\": 1}, \"type\": \""
				+ QueryOptionPage.class.getName() + "\"}", str);
	}

	public void testRestoreQueryOption()
	{
		QueryOption optRestored =
			(QueryOption) JsonSerializer
				.deserialize(
					QueryOption.class,
					Json.loads("{\"type\":\""
						+ QueryOptionPage.class.getName()
						+ "\", \"value\": {\"pageType\": \"TEMPORARY\", \"state\": \"PASSIVE\", \"pageSize\": 0, \"type\": 1} }"));
		Query<PersonLongAutoID> query = pm.createQuery(PersonLongAutoID.class);
	...
	public void testDumpRestoreQueryData()
	{
		Query<Discovery> query =
			pm.createQuery(Discovery.class).filter("name", "test").order("name").join("discoverer", "firstName");
	...
	public void testJoinAnnotationDouble()
	{
		...
		List<Discovery4Join2> res = pm.createQuery(Discovery4Join2.class).order("id").fetch();
		assertEquals(4, res.size());
		assertEquals(radioactivity, res.get(0));
		assertEquals(relativity, res.get(1));
		assertEquals(foo, res.get(2));
	...
	public void testBatchUpdate()
	{
		...
		assertEquals(discs.length, nb);
		res = pm.createQuery(Discovery.class).order("id").fetch();
		int i = 0;
		for (Discovery disc : res)
		{
			assertEquals(discs[i++], disc);
	...
	public void testNoColumn()
	{
		...
		List<DiscoveryNoColumn> res = pm.createQuery(DiscoveryNoColumn.class).order("id").fetch();
		assertEquals(4, res.size());
		assertEquals(radioactivity, res.get(0));
		assertEquals(relativity, res.get(1));
		assertEquals(foo, res.get(2));
	...
	public void testLifeCycleGet()
	{
		...
		assertEquals(LifeCyclePhase.PRE_FETCH.toString() + " " + LifeCyclePhase.POST_FETCH.toString() + " ",
			lifeCyclePhase);
	}
	...
	public void testLifeCycleInsert()
	{
		...
		assertEquals(LifeCyclePhase.PRE_INSERT.toString() + " " + LifeCyclePhase.POST_INSERT.toString() + " ",
			lifeCyclePhase);
	}
	...
	public void testLifeCycleDelete()
	{
		...
		assertEquals(LifeCyclePhase.PRE_DELETE.toString() + " " + LifeCyclePhase.POST_DELETE.toString() + " ",
			lifeCyclePhase);
	}
	...
	public void testLifeCycleUpdate()
	{
		...
		assertEquals(LifeCyclePhase.PRE_UPDATE.toString() + " " + LifeCyclePhase.POST_UPDATE.toString() + " ",
			lifeCyclePhase);
	}
	...
	public void testLifeCycleSave()
	{
		...
		assertEquals(LifeCyclePhase.PRE_SAVE.toString() + " " + LifeCyclePhase.POST_SAVE.toString() + " ",
			lifeCyclePhase);
	}
	...
	{
		BigDecimalModel bigdec = new BigDecimalModel(new BigDecimal("123456789.0123456890"));
		pm.insert(bigdec);

		BigDecimalModel bigdec2 = pm.getByKey(BigDecimalModel.class, bigdec.id);
		assertEquals(bigdec, bigdec2);

		bigdec = new BigDecimalModel(new BigDecimal("999999999.9999999999"));
		pm.insert(bigdec);
		...
		// -100.5
		bigdec = new BigDecimalModel(new BigDecimal("-100.5000000000"));
		pm.insert(bigdec);
	...
	{
		BigDecimalModelNoPrecision bigdec = new BigDecimalModelNoPrecision(new BigDecimal("123456789.01"));
		pm.insert(bigdec);

		BigDecimalModelNoPrecision bigdec2 = pm.getByKey(BigDecimalModelNoPrecision.class, bigdec.id);
		assertEquals(bigdec, bigdec2);

		bigdec = new BigDecimalModelNoPrecision(new BigDecimal("999999999.99"));
		pm.insert(bigdec);
		...
		// -100.5
		bigdec = new BigDecimalModelNoPrecision(new BigDecimal("-100.50"));
		pm.insert(bigdec);
	...
	{
		BigDecimalStringModel bigdec = new BigDecimalStringModel(new BigDecimal("123456789.0123456890"));
		pm.insert(bigdec);

		BigDecimalStringModel bigdec2 = pm.getByKey(BigDecimalStringModel.class, bigdec.id);
		assertEquals(bigdec, bigdec2);

		bigdec = new BigDecimalStringModel(new BigDecimal("999999999.9999999999"));
		pm.insert(bigdec);
		...
		// -100.5
		bigdec = new BigDecimalStringModel(new BigDecimal("-100.5000000000"));
		pm.insert(bigdec);
	...
	{
		BigDecimalDoubleModel bigdec = new BigDecimalDoubleModel(new BigDecimal("123456789.012345"));
		pm.insert(bigdec);

		BigDecimalDoubleModel bigdec2 = pm.getByKey(BigDecimalDoubleModel.class, bigdec.id);
		assertEquals(bigdec, bigdec2);

		bigdec = new BigDecimalDoubleModel(new BigDecimal("999999999.9999999999"));
		pm.insert(bigdec);
		...
		// -100.5
		bigdec = new BigDecimalDoubleModel(new BigDecimal("-100.5000000000"));
		pm.insert(bigdec);
